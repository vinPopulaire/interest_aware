function [clusters, total_system_energy_consumed, total_power_info_transmission, metric_iaf1, metric_iaf2, total_power_info_transmission_only_devices, iaf1, iaf2] = interest_aware_simulation(D, ID, E, E_i, G, params)
...
for round = 1:num_rounds
    ...
    powers_info_transmission_clusterheads = find_powers_to_send_info(powers_requested, clusterheads, E, params);
    ...
    % to other devices
    E = decrease_energy_availability(E, clusterheads, powers_info_transmission_clusterheads, 'info', params);
    ...
    powers_info_transmission = find_powers_devices_will_actually_send(powers_info_transmission_clusterheads, power_from_clusterheads, powers_requested, clusters, clusterheads, G, params);

    round_power_info_transmission(round) = sum(powers_info_transmission);
    
    round_power_info_transmission_only_devices(round) = sum(powers_info_transmission - powers_info_transmission_clusterheads);

    total_system_energy_consumed(round) = total_energy - sum(E);
    
    [round_metric_iaf1(round), round_metric_iaf2(round), iaf1(round), iaf2(round)] = calculate_metric_with_interest(powers_info_transmission, E_i, clusters, clusterheads);
end
...
total_power_info_transmission_only_devices = cumsum(round_power_info_transmission_only_devices);
